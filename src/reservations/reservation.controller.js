...
export const createReservation = async (req, res) => {
  try {
    ...
    // Soporte para payload antiguo: reservationDate + HH:mm
    ...
    const parsedStartTime =
      startTime && String(startTime).includes(':') && reservationDate
        ? buildDateTime(reservationDate, startTime)
        : new Date(startTime);

    const parsedEndTime =
      endTime && String(endTime).includes(':') && reservationDate
        ? buildDateTime(reservationDate, endTime)
        : new Date(endTime);

    if (!userId || !fieldId || !parsedStartTime || !parsedEndTime) {
      return res.status(400).json({
        success: false,
        message: 'userId, fieldId, startTime y endTime son requeridos',
      });
    }
